...
class Point:
    ...
    def __init__(self, x, y, a, b):
        ...
        if y**2 != x**3 + self.a * x + self.b:
            raise RuntimeError('Not a point on the curve {} vs {}'.format(y**2, x**3 + self.a * x + self.b))
        self.x = x
        self.y = y
    ...
    def __add__(self, other):
        ...
        if self.x == other.x:
            if self.y != other.y:
                # point at infinity
                return self.__class__(None, None, a=self.a, b=self.b)
...
class S256Point(Point):
    ...
    def double(self):
        ...
        x = s**2 - two * self.x
        y = s * (self.x - x) - self.y
        return S256Point(x=x.num, y=y.num)
